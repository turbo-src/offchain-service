...
async function getVotes(
	...
	try {
		...
		response.forkBranch =
			data.fork_branch || "no data for forkBranch in engine";
		response.childDefaultHash =
			data.childDefaultHash || "no data for childDefaultHash in engine";
		response.defaultHash =
			data.defaultHash || "no data for defaultHash in engine";
		...
		let quorum = data.quorum || 0.5;
		const totalVotes = data.noTokenAmount + data.yesTokenAmount;
		const totalPossibleVotes = 1_000_000;
		const quorumPercent =
			(totalVotes / totalPossibleVotes) * 100 * (1 / quorum);
		const totalVotePercent = quorumPercent < 1 ? quorumPercent : Math.floor(quorumPercent);


		//Set pull request vote totals data
		response.voteData.voteTotals.totalNoVotes =
			parseInt(data.noTokenAmount) || 0;
		response.voteData.voteTotals.totalYesVotes =
			parseInt(data.yesTokenAmount) || 0;
		response.voteData.voteTotals.totalVotes =
			response.voteData.voteTotals.totalNoVotes +
				response.voteData.voteTotals.totalYesVotes || 0;
		response.voteData.voteTotals.yesPercent =
			parseInt(data.yesTokenAmount) / 1_000_000 || 0;
		response.voteData.voteTotals.noPercent =
			parseInt(data.noTokenAmount) / 1_000_000 || 0;
		response.voteData.voteTotals.totalVotePercent =
			totalVotePercent < 100 ? totalVotePercent : 100;
		response.voteData.voteTotals.quorum = parseInt(data.quorum) || 0.5;
