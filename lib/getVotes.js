...
async function getVotes(
  ...
  try {
    ...
    let response = {
      ...
      forkBranch: data.forkBranch || 'no data for forkBranch in engine',
    ...
    response.voteData.voteTotals.totalVotes = Math.round(response.voteData.voteTotals.totalNoVotes + response.voteData.voteTotals.totalYesVotes) || 0
    response.voteData.voteTotals.yesPercent = Math.round(parseInt(data.yesTokenAmount) / 1000000) || 0
    response.voteData.voteTotals.noPercent = Math.round(parseInt(data.noTokenAmount) / 1000000) || 0
