import { Repo, Contributor, PullRequest, Vote } from "../db/index.js";
import getContributorTokenAmount from "./getContributorTokenAmount.js";
import getVoteYesTotals from "./state/getVoteNoTotals.js";
...
export default async function setVote(
  ...
  try {
    ...
    let pullRequest = await PullRequest.findOne({
      where: { pr_id: pr_id, repoId: repo.id },
    });
    ...
    if (side === "yes") {
      ...
      console.log("data", pullRequest.yesTokenAmount, tokens, newTotal);
