...
async function setVote(
  ...
  try {
    ...
    // Filter by Repo > PullRequest > Vote for efficiency.
    let data = await Repo.findOne({
      where: { repo_id: repo_id },
      include: {model: PullRequest,
        include: {
          model : Vote
      }
    }
    ...
    const pullRequests = obj.pullrequests;
    const curPullRequest = pullRequests.find(pr => pr.defaultHash === defaultHash);
